...
static void showMetadata(SnifferPacket *snifferPacket) {
    ...
    if(SSID != ""){
        ...
        Serial.print("\n\r---------\n\rSSID: ");
        Serial.print(sniffedRouters[strSSID].SSID);
        Serial.print("  ");
...
void setup() {
    ...
    Serial.print("device post status: ");
...
void loop() {
    if (infoFlag == 1) {
        ...
        http.addHeader("Content-type", "application/json");
        int httpCode = http.POST(json);
        Serial.print("packets post status: ");
        Serial.println(httpCode);
        http.end();
        ...
        Serial.println("Disconnected successfuly.");
